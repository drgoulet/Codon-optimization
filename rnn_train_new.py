...
print(len(dna_list_pre))

dna_list = dna_list_pre[:35000]
aa_list = aa_list_pre[:35000]
...
def model_embed(input_shape, output_sequence_length, aa_vocab_size, dna_vocab_size):
    ...
    model = Sequential()
    model.add(Embedding(aa_vocab_size, 128, input_length=input_shape[1], input_shape=input_shape[1:]))
...
callbacks = [EarlyStopping(monitor='val_loss', patience=5), ModelCheckpoint('keras_model_combined_newnew.h5')]

my_model.fit(tmp_x, preproc_dna, batch_size=16, epochs=20, validation_split=0.2, callbacks=callbacks)

print(logits_to_text(my_model.predict(tmp_x[:1])[0], dna_tokenizer))

my_model.save('keras_model_combined_final_newnew.h5')
